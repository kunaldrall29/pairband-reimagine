	...
	_proto.init = function init(vars) {
		...
		var id, onStopDelayedCall, dragged, moved, wheeled, locked, axis, self = this, prevDeltaX = 0, prevDeltaY = 0, passive = vars.passive || !preventDefault && vars.passive !== false, scrollFuncX = _getScrollFunc(target, _horizontal), scrollFuncY = _getScrollFunc(target, _vertical), scrollX = scrollFuncX(), scrollY = scrollFuncY(), limitToTouch = ~type.indexOf("touch") && !~type.indexOf("pointer") && _eventTypes[0] === "pointerdown", isViewport = _isViewport$1(target), ownerDoc = target.ownerDocument || _doc$3, deltaX = [
			...
			if (!isDragNotClick && isTrackingDrag) {
				self._vx.reset();
				self._vy.reset();
				if (preventDefault && allowClicks) gsap$2.delayedCall(.08, function() {
		...
		onStopDelayedCall = self._dc = gsap$2.delayedCall(onStopDelay || .25, onStopFunc).pause();
		self.deltaX = self.deltaY = 0;
...
* @author: Jack Doyle, jack@greensock.com
*/
var ScrollTrigger_exports = /* @__PURE__ */ __exportAll({
...
var _passThrough$1 = function _passThrough(v) {
	return v;
};
...
var _callback$1 = function _callback(self, func) {
	if (self.enabled) {
...
var _createMarker = function _createMarker(type, name, container, direction, _ref4, offset, matchWidthEl, containerAnimation) {
	...
	var e = _doc$2.createElement("div"), useFixedPosition = _isViewport(container) || _getProxyProp(container, "pinType") === "fixed", isScroller = type.indexOf("scroller") !== -1, parent = useFixedPosition ? _body : container, isStart = type.indexOf("start") !== -1, color = isStart ? startColor : endColor, css = "border-color:" + color + ";font-size:" + fontSize + ";color:" + color + ";font-weight:" + fontWeight + ";pointer-events:none;white-space:nowrap;font-family:sans-serif,Arial;z-index:1000;padding:4px 8px;border-width:0;border-style:solid;";
...
var _onResize = function _onResize(force) {
	_scrollers.cache++;
	(force === true || !_refreshing && !_ignoreResize && !_doc$2.fullscreenElement && !_doc$2.webkitFullscreenElement && (!_ignoreMobileResize || _baseScreenWidth !== _win$2.innerWidth || Math.abs(_win$2.innerHeight - _baseScreenHeight) > _win$2.innerHeight * .25)) && _resizeDelay.restart(true);
};
var _listeners$1 = {};
var _emptyArray$1 = [];
...
var _setState = function _setState(state) {
	if (state) {
		...
		for (; i < l; i += 2) {
			...
			else if (style[p]) style.removeProperty(p.replace(_capsExp$1, "-$1").toLowerCase());
		}
	}
};
...
var _parsePosition$1 = function _parsePosition(value, trigger, scrollerSize, direction, scroll, marker, markerScroller, self, scrollerBounds, borderWidth, useFixedPosition, scrollerMax, containerAnimation, clampZeroProp) {
	_isFunction$1(value) && (value = value(self));
	if (_isString$1(value) && value.substr(0, 3) === "max") value = scrollerMax + (value.charAt(4) === "=" ? _offsetToPx("0" + value.substr(3), scrollerSize) : 0);
...
var ScrollTrigger = /*#__PURE__*/ function() {
	...
	_proto.init = function init(vars, animation) {
		...
		vars = _setDefaults$1(_isString$1(vars) || _isNumber$1(vars) || vars.nodeType ? { trigger: vars } : vars, _defaults$1);
		var _vars = vars, onUpdate = _vars.onUpdate, toggleClass = _vars.toggleClass, id = _vars.id, onToggle = _vars.onToggle, onRefresh = _vars.onRefresh, scrub = _vars.scrub, trigger = _vars.trigger, pin = _vars.pin, pinSpacing = _vars.pinSpacing, invalidateOnRefresh = _vars.invalidateOnRefresh, anticipatePin = _vars.anticipatePin, onScrubComplete = _vars.onScrubComplete, onSnapComplete = _vars.onSnapComplete, once = _vars.once, snap = _vars.snap, pinReparent = _vars.pinReparent, pinSpacer = _vars.pinSpacer, containerAnimation = _vars.containerAnimation, fastScrollEnd = _vars.fastScrollEnd, preventOverlaps = _vars.preventOverlaps, direction = vars.horizontal || vars.containerAnimation && vars.horizontal !== false ? _horizontal : _vertical, isToggle = !scrub && scrub !== 0, scroller = _getTarget(vars.scroller || _win$2), scrollerCache = gsap$1.core.getCache(scroller), isViewport = _isViewport(scroller), useFixedPosition = ("pinType" in vars ? vars.pinType : _getProxyProp(scroller, "pinType") || isViewport && "fixed") === "fixed", callbacks = [
			vars.onEnter,
			vars.onLeave,
			vars.onEnterBack,
			vars.onLeaveBack
		], toggleActions = isToggle && vars.toggleActions.split(" "), markers = "markers" in vars ? vars.markers : _defaults$1.markers, borderWidth = isViewport ? 0 : parseFloat(_getComputedStyle(scroller)["border" + direction.p2 + _Width]) || 0, self = this, onRefreshInit = vars.onRefreshInit && function() {
		...
		if (snap) {
			if (!_isObject$1(snap) || snap.push) snap = { snapTo: snap };
			"scrollBehavior" in _body.style && gsap$1.set(isViewport ? [_body, _docEl] : scroller, { scrollBehavior: "auto" });
			...
			snapDelayedCall = gsap$1.delayedCall(snap.delay || scrubSmooth / 2 || .1, function() {
				var scroll = scrollFunc(), refreshedRecently = _getTime() - lastRefresh < 500, tween = tweenTo.tween;
				if ((refreshedRecently || Math.abs(self.getVelocity()) < 10) && !tween && !_pointerIsDown && lastSnap !== scroll) {
					var progress = (scroll - start) / change, totalProgress = animation && !isToggle ? animation.totalProgress() : progress, velocity = refreshedRecently ? 0 : (totalProgress - snap2) / (_getTime() - _time2) * 1e3 || 0, change1 = gsap$1.utils.clamp(-progress, 1 - progress, _abs(velocity / 2) * velocity / .185), naturalEnd = progress + (snap.inertia === false ? 0 : change1), endValue, endScroll, _snap = snap, onStart = _snap.onStart, _onInterrupt = _snap.onInterrupt, _onComplete = _snap.onComplete;
		...
		if (pin) {
			...
			vars.force3D !== false && gsap$1.set(pin, { force3D: true });
		...
		if (markers) {
			...
			if (!useFixedPosition && !(_proxies.length && _getProxyProp(scroller, "fixedMarkers") === true)) {
				...
				gsap$1.set([markerStartTrigger, markerEndTrigger], { force3D: true });
		...
		self.refresh = function(soft, force, position, pinOffset) {
			...
			start = _parsePosition$1(parsedStart, trigger, size, direction, scrollFunc(), markerStart, markerStartTrigger, self, scrollerBounds, borderWidth, useFixedPosition, max, containerAnimation, self._startClamp && "_startClamp") || (pin ? -.001 : 0);
			_isFunction$1(parsedEnd) && (parsedEnd = parsedEnd(self));
			if (_isString$1(parsedEnd) && !parsedEnd.indexOf("+=")) {
				if (~parsedEnd.indexOf(" ")) parsedEnd = (_isString$1(parsedStart) ? parsedStart.split(" ")[0] : "") + parsedEnd;
				else {
					offset = _offsetToPx(parsedEnd.substr(2), size);
					parsedEnd = _isString$1(parsedStart) ? parsedStart : (containerAnimation ? gsap$1.utils.mapRange(0, containerAnimation.duration(), containerAnimation.scrollTrigger.start, containerAnimation.scrollTrigger.end, start) : start) + offset;
					parsedEndTrigger = trigger;
				}
			}
			parsedEnd = _parseClamp(parsedEnd, "end", self);
			end = Math.max(start, _parsePosition$1(parsedEnd || (parsedEndTrigger ? "100% 0" : max), parsedEndTrigger, size, direction, scrollFunc() + offset, markerEnd, markerEndTrigger, self, scrollerBounds, borderWidth, useFixedPosition, max, containerAnimation, self._endClamp && "_endClamp")) || -.001;
			offset = 0;
			i = triggerIndex;
			while (i--) {
				curTrigger = _triggers[i];
			...
			if (isFirstRefresh || prevProgress !== self.progress || containerAnimation || invalidateOnRefresh || animation && !animation._initted) {
				animation && !isToggle && animation.totalProgress(containerAnimation && start < -.001 && !prevProgress ? gsap$1.utils.normalize(start, end, 0) : prevProgress, true);
		...
		self.update = function(reset, recordVelocity, forceFake) {
			...
			if (clipped !== prevProgress && self.enabled) {
				...
				if (pin) {
					...
					if (!useFixedPosition) pinSetter(_round$1(pinStart + pinChange * clipped));
		...
		if (animation && animation.add && !change) {
			...
			gsap$1.delayedCall(.01, self.update);
			change = .01;
			start = end = 0;
		} else self.refresh();
	...
	ScrollTrigger.defaults = function defaults(config) {
		if (config) for (var p in config) _defaults$1[p] = config[p];
		return _defaults$1;
	};
	ScrollTrigger.disable = function disable(reset, kill) {
		...
		_removeListener(_win$2, "wheel", _onScroll);
		_removeListener(_doc$2, "scroll", _onScroll);
		clearInterval(_syncInterval);
		_removeListener(_doc$2, "touchcancel", _passThrough$1);
		_removeListener(_body, "touchstart", _passThrough$1);
		_multiListener(_removeListener, _doc$2, "pointerdown,touchstart,mousedown", _pointerDownHandler);
		_multiListener(_removeListener, _doc$2, "pointerup,touchend,mouseup", _pointerUpHandler);
		_resizeDelay.kill();
	...
	ScrollTrigger.enable = function enable() {
		...
		if (gsap$1) {
			...
			if (_body) {
				...
				_setBaseDimensions();
				_addListener(_doc$2, "scroll", _onScroll);
				...
				_addListener(_doc$2, "touchcancel", _passThrough$1);
				_addListener(_body, "touchstart", _passThrough$1);
				_multiListener(_addListener, _doc$2, "pointerdown,touchstart,mousedown", _pointerDownHandler);
				_multiListener(_addListener, _doc$2, "pointerup,touchend,mouseup", _pointerUpHandler);
	...
	ScrollTrigger.scrollerProxy = function scrollerProxy(target, vars) {
		...
		if (vars) isViewport ? _proxies.unshift(_win$2, vars, _body, vars, _docEl, vars) : _proxies.unshift(t, vars);
	};
...
ScrollTrigger.batch = function(targets, vars) {
	...
	for (p in vars) varsCopy[p] = p.substr(0, 2) === "on" && _isFunction$1(vars[p]) && p !== "onRefreshInit" ? proxyCallback(p, vars[p]) : vars[p];
...
var _inputObserver = function _inputObserver(target, type, inputs, nested) {
	return Observer.create({
		...
		onEnable: function onEnable() {
			return inputs && _addListener(_doc$2, Observer.eventTypes[0], _captureInputs, false, true);
		},
		onDisable: function onDisable() {
			return _removeListener(_doc$2, Observer.eventTypes[0], _captureInputs, true);
		}
	});
};
...
var _getScrollNormalizer = function _getScrollNormalizer(vars) {
	...
	}, scrollClampX = _passThrough$1, scrollClampY = _passThrough$1, updateClamps = function updateClamps() {
	...
	content && gsap$1.set(content, { y: "+=0" });
	...
	vars.onEnable = function() {
		...
		_addListener(_win$2, "resize", onResize);
	...
	vars.onDisable = function() {
		...
		_removeListener(_win$2, "resize", onResize);
...
ScrollTrigger.sort = function(func) {
	if (_isFunction$1(func)) return _triggers.sort(func);
...
var gsap_exports = /* @__PURE__ */ __exportAll({
	CSSPlugin: () => CSSPlugin,
	default: () => gsapWithCSS,
	gsap: () => gsapWithCSS
});
...
//#endregion
export { ScrollTrigger_exports as n, gsap_exports as t };
